...
uint8_t MMU::Read(Addr address) {
    ...
    SPDLOG_TRACE("MMU read from RAM address {:#x} value {}", address, value);
    return value;
}

void MMU::Write(Addr address, uint8_t value) {
    if (address > 0x4020) {
        SPDLOG_TRACE("MMU delegating write to cartridge address {:#x} value {}", address, value);
...
uint8_t& MMU::GetAddRef(Addr address) {
    ...
    if (address < 0x2000) {
        ...
        SPDLOG_TRACE("MMU writing to RAM address {:#x}, effective address {:#x}", address, effectiveAddress);
        return ram[effectiveAddress];
    } else if (address >= 0x2000 && address < 0x4000) {
        auto effectiveRegister = (address - 0x2000) % 8;
        SPDLOG_TRACE("MMU writing address {:#x}, PPU register {}", address, effectiveRegister);
        return ppuRegisters[(address - 0x2000) % 8];
    } else if (address >= 0x4000 && address < 0x4018) {
        auto effectiveRegister = (address - 0x4000) % 24;
        SPDLOG_TRACE("MMU writing address {:#x} to APU or I/O register {}", address, effectiveRegister);
        return apuRegisters[effectiveRegister];
    } else if (address >= 0x4018 && address < 0x4020) {
        auto effectiveRegister = (address - 0x4018) % 8;
        SPDLOG_TRACE("MMU writing to disabled APU or I/O register {}", address);
